...
function checkUser() {
  ...
  if (userId && nickname) {
    // Trust local storage first, then verify in background
    showUserUI(nickname);
    // Verify user still exists on server (non-blocking)
    fetch('/api/user/' + userId)
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (!data.success) {
          // User not found on server, clear local but don't show modal
      ...
      .catch(function() {
        // Offline, trust local - no action needed
      });
  } else {
    showSetupModal();
  }
...
async function registerUser() {
  ...
  formData.append('qrcode', pendingQrcodeFile);
