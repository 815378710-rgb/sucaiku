...
// --- API helper ---
function getApiBase() {
  // Detect if accessed via /zhongcao/ prefix
  var path = window.location.pathname;
  if (path.startsWith('/zhongcao/')) {
    return '/zhongcao';
  }
  return '';
}
...
async function adminFetch(url, options) {
  ...
  var res = await fetch(apiBase + url, Object.assign({}, options, { headers: headers }));
  if (res.status === 401) {
    logout();
...
async function publishMaterial(e) {
  ...
  try {
    var res = await adminFetch('/api/admin/materials', { method: 'POST', body: formData });
    var data = await res.json();
    if (data.success) {
      showToast('🎉 发布成功~');
...
function renderOrders() {
  ...
  list.innerHTML = orders.map(function(o) {
    ...
    var postUrlAttr = o.postUrl ? (' data-post-url="' + escapeHtml(o.postUrl) + '"') : '';
    var submitNoteAttr = o.submitNote ? (' data-submit-note="' + escapeHtml(o.submitNote) + '"') : '';
    var qrcodeAttr = o.userQrcode ? (' data-qrcode="' + escapeHtml(o.userQrcode) + '"') : '';
    var userWechatAttr = o.userWechat ? (' data-user-wechat="' + escapeHtml(o.userWechat) + '"') : '';
    return '<div class="admin-item">' +
      '<div class="admin-item-header">' +
      ...
      '<div class="admin-item-actions">' +
        (o.status === 'submitted' ? '<button class="btn-sm btn-review" onclick="openReview(this)" data-order-id="' + o.id + '" data-title="' + escapeHtml(o.materialTitle) + '" data-user="' + escapeHtml(o.userName) + '" data-reward="' + o.reward + '"' + postUrlAttr + submitNoteAttr + qrcodeAttr + userWechatAttr + '>📋 审核</button>' : '') +
...
function openReview(btn) {
  ...
  var userQrcode = btn.dataset.qrcode || '';
  var userWechat = btn.dataset.userWechat || '';
  
  var content = '<div class="review-info">' +
    '<div class="review-title">' + escapeHtml(title) + '</div>' +
    '<div class="review-meta">接单人：' + escapeHtml(userName) + (userWechat ? ' (' + escapeHtml(userWechat) + ')' : '') + ' ｜ 赏金：¥' + reward + '</div>';
  
  if (postUrl) {
    content += '<div style="margin-top:12px;"><a href="' + escapeHtml(postUrl) + '" target="_blank" style="color:#2e7d32;word-break:break-all;">🔗 ' + escapeHtml(postUrl) + '</a></div>';
  }
  if (submitNote) {
    content += '<div style="margin-top:8px;color:#666;">📝 ' + escapeHtml(submitNote) + '</div>';
  }
  if (userQrcode) {
    content += '<div style="margin-top:12px;"><img src="' + escapeHtml(userQrcode) + '" style="width:100px;height:100px;object-fit:contain;border-radius:8px;border:2px solid var(--border);" alt="收款码"></div>';
  }
  content += '</div>';
